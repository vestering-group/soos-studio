...
**Theme:** light · **Implementatie:** de tokens hieronder staan als Tailwind `@theme` in [`src/styles/global.css`](src/styles/global.css). Als hoofdlettertype gebruikt de implementatie **DM Sans** (moderne grotesk, 400 voor body en 500 voor koppen) in plaats van de serif uit deze referentie; DM Mono blijft het label-lettertype. De lime-accentkleur (`#e3f794`) is vervangen door **blush rosé `#f2c4ce`** — warmer en passender bij het merk. Alle overige regels (schaarste van het accent, 2px hoeken, geen schaduwen) blijven onverkort gelden.
