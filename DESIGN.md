...
**Theme:** light · **Implementatie:** de tokens hieronder staan als Tailwind `@theme` in [`src/styles/global.css`](src/styles/global.css). Beausite (commercieel) is in de implementatie vervangen door Instrument Serif; DM Mono wordt ongewijzigd gebruikt. De lime-accentkleur (`#e3f794`) uit deze referentie is in de implementatie vervangen door **blush rosé `#f2c4ce`** — warmer en passender bij het merk; alle overige regels (schaarste van het accent, 2px hoeken, geen schaduwen) blijven onverkort gelden.
